import { balanceOf, tokenName, tokenSymbol, getContract, transferToken, isWalletConnected, mintToken, connectWallet, decimalToken } from './helpers';
...
function App() {
  ...
  const handleTransferToken = async (e: any) => {
    ...
    try {
      console.log("amount", Number(token), address.toString())
      const result = await transferToken(address, Number(token))
      console.log("result ", result)
      if (result === true) {
  ...
  return (
    <div className='min-h-screen bg-gray-100 flex flex-col justify-center items-center'>
      ...
      <main className="flex flex-col items-center mt-10 space-y-8">
        ...
        <form className="max-w-md bg-stone-100 rounded-lg shadow-md px-8 py-6 space-y-4" onSubmit={handleTransferToken}>
  <div>
